...
select cliente.nombre_cliente, concat(empleado.nombre, empleado.apellido1) as nom_y_apell
from cliente inner join empleado
on cliente.codigo_cliente = empleado.codigo_empleado;



# 2. SELECT cliente.nombre_cliente, CONCAT(empleado.nombre, ' ', empleado.apellido1) AS nombre_representante
...
SELECT cliente.nombre_cliente, 
       CONCAT(empleado.nombre, ' ', empleado.apellido1) AS nombre_representante,
...
FROM cliente
INNER JOIN empleado ON cliente.codigo_representante = empleado.codigo_empleado
INNER JOIN oficina ON empleado.codigo_oficina = oficina.codigo_oficina
...
SELECT e1.nombre AS empleado,CONCAT(e2.nombre, ' ', e2.apellido1) AS jefe,CONCAT(e3.nombre, ' ', e3.apellido1) AS jefe_del_jefe
FROM empleado AS e1
...
# por el número de unidades vendidas de la tabla detalle_pedido. El IVA es el 21 % de la base imponible, 
# y el total la suma de los dos campos anteriores.

select sum(detalle_pedido.precio_unidad *
detalle_pedido.cantidad) as base_imponible,
sum(detalle_pedido.precio_unidad *
detalle_pedido.cantidad) * 0.21 as IVA,
(sum(detalle_pedido.precio_unidad *
detalle_pedido.cantidad) + sum(detalle_pedido.precio_unidad *
detalle_pedido.cantidad) * 0.21) as total
from detalle_pedido;


# 19 
select sum(total) as pagos,
...
where p.codigo_producto = producto.codigo_producto)


describe producto;
SELECT
    codigo_producto, -- o cualquier otra columna identificativa
    nombre,
    cantidad_en_stock
FROM
    producto
WHERE
    cantidad_en_stock = (SELECT MAX(cantidad_en_stock) FROM producto);
...
SELECT 
    ...
    COUNT(DISTINCT codigo_producto) AS Numero_Productos_Diferentes_Pedido
FROM 
    detalle_pedido
GROUP BY 
    codigo_pedido
HAVING 
    COUNT(DISTINCT codigo_producto) > 5
ORDER BY 
    Numero_Productos_Diferentes_Pedido ASC;


SELECT Codigo_Pedido, COUNT(DISTINCT Codigo_Producto) AS Numero_Productos_Diferentes_Pedido
FROM Detalles_Pedido
GROUP BY Codigo_Pedido
HAVING Numero_Productos_Diferentes_Pedido > 5
ORDER BY Numero_Productos_Diferentes_Pedido ASC;

# 3 
SELECT 
    nombre_cliente AS Cliente, 
    limite_credito AS Mayor_Límite_Credito
FROM 
    cliente
WHERE 
    limite_credito = (
...
SELECT 
    ...
    p.fecha_esperada AS Fecha_Esperada_Entrega, 
    p.fecha_entrega AS Fecha_Entrega, 
    DATEDIFF(p.fecha_entrega, p.fecha_esperada) AS Dias_Retardo
FROM 
    cliente c
JOIN 
    pedido p 
